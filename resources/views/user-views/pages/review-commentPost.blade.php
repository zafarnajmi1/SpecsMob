@extends('layouts.app') @section('title', "$review->name") @section('sidebar')
@include('partials.aside') @endsection @section('content')

<!-- Review Header -->
<div
    class="hidden lg:block relative w-full max-w-full h-72 md:h-[314px] bg-cover bg-center box-border"
...
>
    ...
    <div class="absolute bottom-0 left-0 w-full z-10">
        <h1
            class="text-white px-4 text-3xl md:text-4xl font-bold drop-shadow-xl mb-2"
        >
            {{ $review->title }}
        </h1>
...
<div
    class="hidden lg:flex justify-between items-center border-t border-gray-400 shadow text-white px-4 md:px-6 mt-3 py-2 bg-[#f0f0f0] backdrop-blur-sm"
>
    <a
        href="#"
    ...
    <div>
        ...
        <strong
            class="bg-[#555] border border-[#333] text-[#f0f0f0] px-[11px] py-[5px] text-[13px]"
            >1</strong
        >
    </div>
</div>
...
<section class="lg:hidden">
    <div class="flex gap-3 pt-3">
        ...
        >
        <a
            href="{{ route('comment.post', ['slug' => $review->slug, 'id' => $review->id, 'type' => 'review'])}}"
            class="uppercase text-[14px] font-bold bg-[#F9A13D] text-white px-3 py-1 rounded-sm shadow"
            >post your comment</a
        >
    </div>
    <div>
        <h2 class="font-bold text-[#F9A13D]">{{$review->title}}</h2>
        <p>{{$review->published_at->format('F d, Y')}}</p>
    </div>
</section>
...
<div class="bg-[#f0f0f0]">
    ...
    <div class="p-5 mt-4 normal-text user-submit">
        ...
        >
            ...
            {{-- USER INFO --}}
            <div
                class="mb-4 flex justify-between items-center border-b border-b-[#d4d4d4] pb-[9px] pt-[3px]"
            >
                <div class="text-green-700 text-lg">Logged in</div>
                <a
                    href="{{ route('logout') }}"
            ...
            <fieldset class="flex flex-col gap-2 items-end mb-4">
                ...
                <a
                    href="{{ route('user.account.manage', Auth::user()->username) }}"
                    class="flex justify-center items-center font-bold px-3 py-1 text-[14px] bg-[#f9f9f9] text-[#555] border border-[#d1d1d1] hover:bg-[#F9A13D] hover:text-white transition uppercase"
                >
                    <span>Manage</span>
                </a>
            ...
            <fieldset class="mb-4">
                ...
                <textarea
                    name="comment"
                    rows="8"
                    class="w-full border-1 border-[#a9a9a9] px-3 py- bg-white"
                    required
    ...
    <div class="bg-white border border-[#d1d1d1] p-5 mt-4 text-center">
        <p class="mb-3 text-[#555]">
            You must be logged in to post an opinion.
        </p>
        <a
            href="{{ route('login') }}"
            class="bg-[#F9A13D] text-white px-4 py-2 inline-block"
        >
            Login
        </a>
    </div>
...
{{-- ================= Posting Rules ================= --}}
<div
    class="max-w-[900px] mx-auto mt-8 bg-white border border-[#d1d1d1] p-6 text-[#555]"
>
    <h4 class="text-[20px] font-bold mb-4">Posting rules</h4>

    <ul class="list-disc pl-6 space-y-2 text-[15px] leading-relaxed">
        <li>English only</li>
        <li>Be polite, use common sense, and avoid foul language</li>
        <li>
            No hate speech (race/ethnicity, nationality, sex, sexual
            orientation, disability, religion, political affiliation)
        </li>
        <li>
            No political or religious discussion – this is a tech-focused
            platform
        </li>
        <li>No spam, commercial ads, or referral links</li>
        <li>
            No bashing – deliberately and repeatedly attacking the same brand
        </li>
        <li>
            No trolling – deliberately antagonizing other brand or model users
        </li>
        <li>
            Read before you post. Search before you post. Your question may
            already have an answer
        </li>
        <li>
            Do not share personal information. We are not responsible for misuse
        </li>
    </ul>
...
{{-- ================= Moderation Note ================= --}}
<div
    class="max-w-[900px] mx-auto mt-4 bg-[#f9f9f9] border border-[#d1d1d1] p-4 text-[14px] text-[#666]"
>
    <strong class="text-[#555]">Note:</strong>
    Comments are subject to automated and moderator review. We may hide or
    remove any comment at our discretion. Comment authors are solely responsible
    for their content. Do not post unlawful, abusive, or otherwise inappropriate
    material, or your account may be banned.
</div>
