                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 bg-white border border-gray-300 min-w-[200px]">
                                    Store
                                </th>
                                <!-- Variant columns with fixed width -->
                                @foreach($allVariants as $index => $variant)
                                    <th class="px-4 py-3 text-center text-[14px] font-medium text-black border border-gray-300 min-w-[116px] {{ $index % 2 != 0 ? 'bg-white' : 'bg-[#fafafa]' }}">
                        ...
                        <tbody>
                            @foreach($offersByStore as $storeName => $storeOffers)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 bg-white border border-gray-300">
                                    ...
                                    @foreach($allVariants as $index => $variant)
                                        ...
                                        @endphp
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-center border border-gray-300 {{ $colBgClass }} min-w-[116px]">
                                            @if($variantOffer)
                                                <div class="flex flex-col items-center justify-center h-full">
                                                    <a href="{{ $variantOffer->url ?: '#' }}" 
                                                       target="_blank"
                                                       class="text-[#d50000] font-bold text-[17px] hover:underline transition-colors leading-tight"
                                                       @if($variantOffer->url)
                                                    ...
                                                    @if(!$variantOffer->in_stock)
                                                        <span class="text-xs text-red-600 mt-1 font-medium">Out of stock</span>
                                                    @endif
                                                </div>
                                            @else
    ...
    @endif
        ...
        <div class="mt-8 space-y-3 text-[#292828] mb-4 leading-relaxed text-[13px] px-2">
            <p class="leading-relaxed">
                <span class="font-semibold">Note:</span> The pricing published on this page is meant to be used for general information only. 
            ...
            <p class="leading-relaxed">
                We provide the links for price comparison purposes but as Associates to Amazon and the other stores linked above, 
