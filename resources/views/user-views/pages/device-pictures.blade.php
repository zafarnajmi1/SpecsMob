            ...
            <div class="flex flex-col gap-2 items-start mt-6 w-full">
                ...
                <div class="flex flex-col gap-3 items-center w-full">
                    @forelse($group->images as $img)
                        <img src="{{ Storage::url($img->image_url) }}" alt="{{ $img->caption ?? $group->title }}"
    ...
    <div
        ...
        <div class="flex gap-4 h-full">
            ...
            <a href="{{ route('device.opinions', ['slug' => $device->slug, 'id' => $device->id])}}"
                class="flex items-center gap-1 hover:bg-[#F9A13D] transition px-3">
                <i class="fa-solid fa-newspaper"></i> OPINIONS
            </a>

            <button class="compare-btn flex items-center gap-1 hover:bg-[#F9A13D] transition px-3">
