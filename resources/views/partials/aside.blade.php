...
<div class="w-full bg-white mt-5">
    ...
    <div class="overflow-y-auto max-h-[390px] bg-[#e7e4e4] p-3 grid grid-cols-3 gap-4">
        ...
        @foreach ($latest as $device)
            <a href="{{ $device['url'] }}" class="text-center text-[11px] text-[#555] hover:text-red-600">
        ...
        <ul class="space-y-1">
            @foreach ($comparisons as $index => $item)
                <li class="{{ $loop->even ? 'bg-[#fff2ee]' : '' }} px-2 py-1 transition">
                    <a href="{{ $item['url'] }}"
                       class="text-[12px] text-[#555] hover:text-[#d50000] leading-tight">
                        {{ $item['text'] }}
                    </a>
                </li>
            @endforeach
...
<div class="w-full bg-white mt-6">
    ...
    {{-- Header --}}
    <h4 class="border-l-[11px] border-[#9e9e9e] text-[#555] uppercase font-bold text-[12px] px-4 py-3">
        <a href="https://www.arenaev.com/" target="_blank" class="hover:text-[#d50000]">
            Electric vehicles
        </a>
    </h4>
