        ...
        @if($comment->replies && $comment->replies->count() > 0)
            <div class="mt-4 space-y-4 bg-[#f0f0f0]">
                @foreach($comment->replies as $reply)
                    @include('partials.device-comment', ['comment' => $comment, 'reply' => $reply, 'isReply' => true])
                @endforeach
            </div>
        @endif
    </div>
